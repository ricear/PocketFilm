...
var selected_type = href.indexOf('selected_type') >= 0 ? href.split('selected_type=')[1].split('&')[0] : '全部'
...
$(function () {
    //  获取影视数据
    var more_movie_url = API + "/tv/get/all?type=" + type + "&selected_type=" + selected_type + "&page_index=" + page_index + "&page_size=" + pageSize + "&key_word=" + key_word + "";
    get_movies(more_movie_url, 'more-movies')
    //  获取影视资源数量
    var count_movie_url = API + "/count/get?source_type=tv&type=" + type + "&selected_type=" + selected_type + "&page_index=" + page_index + "&page_size=" + pageSize + "&key_word=" + key_word + "";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (page_index <= 1) {
                ...
                html = html + '<a href="/'+more_type+'?type=' + type + '&selected_type=' + selected_type + '&re_gion=' + region + '&release_date=' + release_date + '&page_index=' + 1 + '&sort_type=' + sort_type + '&key_word=' + key_word + '" class="pagelink_a">首页</a><a href="/'+more_type+'?type=' + type + '&selected_type=' + selected_type + '&re_gion=' + region + '&release_date=' + release_date + '&page_index=' + (page_index - 1) + '&sort_type=' + sort_type + '&key_word=' + key_word + '" class="pagelink_a">上一页</a>'
            }
            html = html
            for (i in pages) {
                var page = pages[i]
                if (page == page_index) {
                    ...
                    html = html + '<a target="_self" class="pagelink_b ee" href="/'+more_type+'?type=' + type + '&selected_type=' + selected_type + '&re_gion=' + region + '&release_date=' + release_date + '&page_index=' + page + '&sort_type=' + sort_type + '&key_word=' + key_word + '">' + page + '</a>'
                }
            }
