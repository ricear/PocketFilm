...
function get_movies(url, id) {
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i in data) {
                movie = data[i]
                html = html + '<div class="qcontainer"><div class="film"><div class="face front"><li class="p2 MPli"><a class="link-hover" target="_blank" href="/play?_id=' + movie._id + '&source_index=0&type_index=0" title="' + movie.name + '"><img class="lazy" data-original="' + movie.src + '" src="' + movie.src + '" alt="' + movie.name + '"><span class="index5njcom titletj5nj"><i>' + movie.name + '</i></span></a></li></div><div class="face back"><li class="p2 MPli"><a class="link-hover" target="_blank" href="/play?_id=' + movie._id + '&source_index=0&type_index=0" title="' + movie.name + '"><img class="lazy" data-original="' + movie.src + '" src="http://5nj.com/template/Datll/images/load.gif" alt="' + movie.name + '"><span class="video-bg"></span><span class="lzbz"><p class="name">' + movie.name + '</p><p class="actor">' + movie.type2 + '</p></span><p class="other"><i></i></p></a></li></div></div></div>'
            }
            $('#' + id).html(html)
        }
    })
