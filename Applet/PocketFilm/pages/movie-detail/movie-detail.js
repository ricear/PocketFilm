...
Page({
  ...
  changeSourceType(e) {
    ...
    that.setData({
      typeIndex: index,
      play_url: that.data.sourceTypes[that.data.typeIndex].url
    })
  },
  ...
  changeSource(e) {
    ...
    that.setData({
      ...
      select: false,
      sourceTypes: that.data.movie.sources[index].types
    })
  },
  ...
  onLoad: function (options) {
    ...
    util.getMovieDetailById(_id).then((data) => {
      if (data.code == 0) {
        var movie = data.data
        var movieListParams = util.movieListParams
        movieListParams.pageSize = that.data.pageSize
        movieListParams.keyWord = movie.name
        util.getMovieList(movieListParams).then((data2) => {
          var play_url = movie.sources.length == 0 ? that.data.play_url : movie.sources[0].types[0].url
          var sourceName = movie.sources.length == 0 ? that.data.sourceTypes : movie.sources[0].name
          var sourceTypes = movie.sources.length == 0 ? that.data.sourceTypes : movie.sources[0].types
